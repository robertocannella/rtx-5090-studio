...
- [Math Notes](MATH-NOTES.md) — public FastAPI/SQLite blog for math notes and worked word problems, authored from the History Generator Configuration UI.
...
| History Generator — Configuration UI | `generator.example.com` (`basic_auth`) | Browser form to configure, start, and track episodes without a chat prompt -- also now the "Math Notes" post-authoring tab, see below |
| ComfyUI | *(private, no hostname — reached only by History Generator)* | Local FLUX.1-schnell image generation, GPU-backed, used by History Generator's `visual_style="images"`/`"thumbnail"` |
...
| Math Notes | `math.example.com` (public) | FastAPI/SQLite blog for math notes and worked word problems, with LaTeX via KaTeX. Posts are created from the Configuration UI's "Math Notes" tab, not by editing files |
| Math Notes — admin API | *(private, no hostname — reached only by the Configuration UI)* | Post create/update/delete, shares Math Notes' SQLite database |
...
Caddy Gateway
   ...
   +-- math.example.com      --> math-notes (public, FastAPI + SQLite)

Docker network: edge (Open WebUI additionally bridges three private networks: ollama_internal and tts_internal — see OLLAMA.md — and historygen_internal, to reach the history-api service — see HISTORY-GENERATOR.md. history-ui, the Configuration UI, also bridges historygen_internal and tts_internal the same way, so it can reach history-api and Kokoro's voice list, plus mathnotes_internal to reach math-notes-api for the "Math Notes" tab — see HISTORY-GENERATOR.md. History Generator (both history-generator and history-api) additionally bridges comfyui_internal, a private network shared only with the standalone comfyui service, for AI-generated segment images — see HISTORY-GENERATOR.md. math-notes-api, the private post-management API behind Math Notes, joins only mathnotes_internal — see MATH-NOTES.md)

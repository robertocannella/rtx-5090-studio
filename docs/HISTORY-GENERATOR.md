...
After a job starts, its `job_id`/`slug` are added to a small tracked-jobs list in `localStorage` and polled (`GET /api/jobs/{id}`, every ~8s) for live progress — segments complete, narration vs. target duration, domain, and (once done) the same `final_video_host_path` the Open WebUI tool reports (see [Getting the finished video](#getting-the-finished-video)): a real path on the server's filesystem, not a browser download link — this UI never serves, streams, or duplicates video bytes, consistent with `history-api` never doing so either. An "All episodes on this server" panel lists every episode via `GET /api/episodes`, independent of what this browser happens to have tracked, so a job started from Open WebUI (or a different browser/device) is still visible here.

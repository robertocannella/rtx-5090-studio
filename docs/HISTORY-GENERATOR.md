...
The color theme is [Nord](https://www.nordtheme.com) dark (`ui/static/style.css`'s `:root` custom properties) -- `--bg`/`--panel`/`--panel-alt`/`--border` are nord0-nord3, `--text` is nord6, `--accent`/`--accent-dim` are the frost blues (nord8/nord10), and `--danger`/`--ok` are nord11/nord14. Every color in the stylesheet is one of these variables (or a small fixed adjustment of one, e.g. `.top-error`'s background) rather than a one-off hex value, so retheming stays a `:root` edit, not a find-and-replace across the file.

The browser-tab icon is `ui/static/favicon.svg` -- a small inline play-triangle mark in the same Nord colors (`#2e3440` background, `#88c0d0` triangle), referenced from `index.html` via a plain `<link rel="icon" type="image/svg+xml">`. No build step or multi-resolution `.ico`/PNG set -- modern browsers render an SVG favicon natively at whatever size the tab needs, and this being an internal authenticated tool means there's no need to support browsers old enough to lack SVG favicon support.
...
**Both panels skip re-rendering entirely when a refresh comes back identical to what's already on screen** (`lastJobsJson`/`lastEpisodesJson` in `app.js`, a plain `JSON.stringify` equality check against the previous poll). This isn't just an optimization: rebuilding the DOM on every 8s tick regardless of whether anything changed was resetting in-progress typing in the YouTube-publish video-ID input, clearing text selection, and generally disrupting an open YouTube-metadata panel every few seconds, even after the "always shows Loading..." flash (above) was fixed -- the rebuild-and-restore both still happened each tick, just back-to-back with no visible gap. Once an episode is finished and nothing about it is changing between polls (the common case while actually reading an open panel), the list is now left completely untouched.
...
### Episodes tab: search, edit, delete

The page is split into two tabs (`#tab-generate`/`#tab-episodes` in `index.html`, switched client-side by `wireTabs()`/`switchTab()` in `app.js` -- a plain `.active` class toggle, no routing, no page reload): **Generate** (the preset bar, job-submission form, and Active & recent jobs panel, unchanged) and **Episodes**, a dedicated listing for every episode `history-api` knows about, independent of job state.

**Search and filter are entirely client-side.** `loadEpisodes()` fetches `GET /api/episodes` on the same 8s interval as before and caches the raw result in `allEpisodes`; a separate `renderEpisodesList()` filters that cached array against the search box (`#episode-search`, matches `title` or `topic`, case-insensitive substring) and three dropdowns (`#episode-domain-filter`, populated from `GET /metadata`'s `domains` list the same way the job form's own domain picker is; `#episode-status-filter`, `done` vs `in-progress` off `final_video_exists`; `#episode-youtube-filter`, `published`/`not-published` off `youtube_video_id`, or `private`/`unlisted`/`public` matched against `youtube_privacy_status` -- see the badges paragraph below for why that field can be stale or absent) before building any DOM. Typing in the search box or changing a filter calls `renderEpisodesList()` directly with no network round-trip and no interaction with the `lastEpisodesJson` skip-if-unchanged check (above) -- that check still guards only the raw 8s fetch, so a slow poll can never stomp on someone mid-search.

**Five separate columns, sortable by clicking a header**: Episode, Length, Segments, Status, and Actions -- previously segment count and status were crammed together into one "Progress" column as a single string. Length shows `narration_seconds` once known (falling back to the requested `target_duration_seconds` while a job is still narrating) formatted as `Xh YYm`/`Xm YYs`/`Xs` (`formatDuration()`); Segments shows `segments_complete`/`segments_total`, or just `N narrated` once `target_reached` (same reasoning as the job cards -- see the `target_reached` note below). Clicking **Episode**, **Length**, **Segments**, or **Status** in the header (`.col-sort-btn`, wired by `wireSortHeaders()`) sorts the filtered list by that column, ascending; clicking the same header again reverses to descending, shown with a &#9650;/&#9660; indicator next to the active column. Sorting, like filtering, is pure client-side array sort over the already-cached `allEpisodes` -- no new endpoint, no server-side ordering. Below ~760px the table collapses to stacked rows (as before) with `data-label` attributes CSS-prefixed onto the Length/Segments/Status values so they stay unambiguous without the column headers.

**The Actions and Episode columns have a fixed width and scroll horizontally instead of wrapping**: Actions can hold a lot (a YouTube button, "Edit", "Delete"), and the Episode column now can too (title, domain tag, Published badge, privacy badge) -- letting either wrap onto a second line pushed that one row taller than every other row in the table, breaking the shared grid's otherwise-uniform row height. Both `.ep-col-actions` and `.ep-col-title` are `white-space: nowrap`/`flex-wrap: nowrap` with `overflow-x: auto` and `min-width: 0` (needed because a grid item's default `min-width: auto` would otherwise refuse to shrink below its content's natural width, defeating the scroll) -- so a row with more content than fits just scrolls sideways within that one cell, and every row stays the same height regardless of how long its title is or how many actions/badges it's showing.

**Why the header and rows share one CSS Grid, not five**: an earlier version gave `.episode-table-header` and every `.episode-row` their own independent `display: grid`, all declaring the same `grid-template-columns` -- which looks right in the CSS but isn't, because `minmax(Npx, auto)` sizes its `auto` component against only the content *in that one grid instance*. With one grid per row, each row (and the header) computed its own column widths from its own content alone, so a long title in one row, a long status string in another, and the header's own text could each land on different actual pixel widths -- nothing lined up. The fix (`#episodes-list.episode-grid`) makes `#episodes-list` itself the only grid container; `.episode-table-header`, `.episode-row-wrap`, and `.episode-row` are all `display: contents`, which removes their own boxes and lets their children (the header cells, and each row's five column divs) become direct children of that one shared grid -- so every `auto` column is now sized against every cell in that column across the header and all rows at once, the way an HTML `<table>` would. `.yt-panel` gets `grid-column: 1 / -1` for the same reason -- as a `display: contents` wrapper's child, it's now a direct grid child too, and needs to explicitly span the full row rather than trying to fit in one column. Per-row borders moved from the (now-boxless) `.episode-row` onto each `.episode-row > div` individually -- five adjacent bottom borders across a row still read as one line at normal viewing distance. Below 760px, `#episodes-list.episode-grid` simply drops to `grid-template-columns: 1fr`, so the exact same markup stacks single-column with no separate mobile DOM structure.

**Renaming an episode**: "Edit" in the Actions column (next to "Delete") swaps the title for an inline `<input>` + Save/Cancel (`startEditEpisodeTitle()`), calling `PATCH /episodes/{slug}` (`api.py:update_episode()`) with `{"title": ...}` on save. Renaming is the only per-row edit right now, but it's grouped under a generic "Edit" action rather than a "Rename" button specifically so a later per-row editable field (if one's ever added) has an obvious home instead of needing its own dedicated button. This only ever changes the manifest's display `title` -- the on-disk `slug` (and therefore every path under `output/<slug>/`, and the Jellyfin hardlink) is immutable, so renaming can't orphan or break anything else. An empty/whitespace-only title is rejected client-side and server-side (422).

**YouTube info lives in a modal, not an inline expanding panel**: clicking "YouTube" in the Actions column -- the same single button whether or not metadata exists yet for that episode, see [Generating on demand](#youtube-metadata) for what the modal shows when it doesn't -- opens `#youtube-modal` -- a centered overlay (`openYoutubeModal()`/`closeYoutubeModal()`/`wireYoutubeModal()`) holding everything the old inline panel had: view/edit metadata, publish to YouTube, and regenerate. It closes on the &times; button, clicking the backdrop, or Escape. This replaced an earlier version where clicking the button expanded a panel inline below that row -- which meant the episode list's own 8s auto-refresh had to explicitly track which row was expanded and restore it after every re-render (`expandedYoutube`, now removed) just to avoid silently collapsing it out from under someone mid-read. A modal sidesteps that entirely: it's a separate DOM node outside `#episodes-list`, so re-rendering the list while it's open doesn't touch it at all. `youtubeCache` (per-slug, populated on first open) still avoids a redundant fetch on repeat opens, and `pollYoutubePublishStatus()`'s "is anyone still looking at this" check moved from `document.body.contains(panel)` to a plain `currentYoutubeModalSlug !== slug` comparison, since the panel's DOM node persists (just hidden) across opens/closes now rather than being torn down.

**Editing YouTube metadata by hand**: inside the modal, "Edit metadata" (`wireYoutubeEditMode()`) swaps the read-only title/category/tags/description into editable fields and a Save/Cancel row, saving via `PATCH /episodes/{slug}/youtube` (see [YouTube Metadata's Manual editing](#youtube-metadata)). Chapters stay read-only in both modes -- they're pure arithmetic, not something to hand-edit (see above). This is for the case where the Ollama-generated title/description needs a small human correction (a fact it got slightly wrong, a title you'd phrase differently) without burning another Ollama call via "Regenerate metadata."

**Deleting an episode**: "Delete" on a row (`startDeleteEpisode()`) opens `#confirm-modal` -- a small, generic "are you sure?" popup (`openConfirmModal()`/`closeConfirmModal()`/`wireConfirmModal()`, the same `.modal-overlay`/`.modal-box` pattern as the YouTube modal, just narrower via `.modal-box-small`) naming the episode's title, rather than a browser `confirm()` dialog (which can't be styled to match, or a native OS dialog is easy to reflexively dismiss without reading) or replacing the row's own Actions column in place (the earlier approach -- which meant reconstructing that cell's real content by hand if the delete then failed, and left the row in a half-transformed state for however long the request took). `openConfirmModal({message, confirmLabel, onConfirm})` is intentionally generic, not delete-specific -- `onConfirm` is an async callback the modal awaits, disabling both buttons and showing "Working..." while it runs, closing itself on success or showing the failure message with both buttons re-enabled so the user can retry or cancel. Confirming calls `DELETE /episodes/{slug}` (`api.py:delete_episode()`), which:

- Refuses (`409`) if a job is currently running for that slug (`_active_slugs`) -- deleting an episode's directory out from under an in-progress pipeline run would corrupt it rather than cleanly stop it.
- Otherwise removes the **entire** episode directory (`shutil.rmtree(paths["root"])`) -- script, audio, images, segment videos, the final video, and the manifest all go, not just the final video. This is a real, irreversible, disk-freeing delete, not a soft "hide from the list."
- Clears any tracked YouTube-upload status for that slug (`_youtube_publish_status.pop(slug, None)`) so a stale "uploading"/"complete" state can't resurface if the same slug is ever regenerated later.
- Validates the slug resolves to a real path under `OUTPUT_DIR` before touching the filesystem (`_safe_episode_root()`, `os.path.realpath` + prefix check) -- defense against a path-traversal slug (`../../etc`) ever reaching `shutil.rmtree`, even though slugs are normally generated by `slugify()` and never come from unvalidated user text in practice.

The Jellyfin hardlink for a deleted episode isn't cleaned up by this call itself -- that's `sync-jellyfin-library.sh`'s job (see its orphan-pruning pass under [Jellyfin Preview](#jellyfin-preview)), which runs within 15 minutes via cron or can be run by hand immediately after a delete.

**Tests**: `tests/test_api_episode_management.py` (rename success/whitespace-trim/empty-rejection/404, delete success/404/409-while-active/publish-status-cleanup, and the path-traversal-rejection cases for `_safe_episode_root()`), plus the corresponding proxy-passthrough tests in `ui/tests/test_main.py` (`PATCH /api/episodes/{slug}`, `DELETE /api/episodes/{slug}`, `PATCH /api/episodes/{slug}/youtube`).
...
**Orphan pruning**: each run also walks `/srv/media/generated-episodes/*.mp4` and removes any hardlink whose source (`output/<slug>/final/<slug>.mp4`) no longer exists — this is what keeps the Jellyfin library in sync with the Configuration UI's [Episodes tab delete](#episodes-tab-search-edit-delete), which removes an episode's directory outright (`shutil.rmtree`) rather than just its final video. Deletion isn't instant in Jellyfin either way — the link disappears as soon as this script next runs (on-demand, or within 15 minutes via cron), but Jellyfin itself only notices a missing file on its own library scan schedule.
...
**Access**: `GET /episodes/{slug}/youtube` (404 until generated) returns the stored dict as-is; `_episode_progress()`'s `youtube_metadata_ready` boolean (on `/episodes` and `/episodes/{slug}`) is what the Configuration UI checks to decide which button to show on that episode's row -- clicking "YouTube" (when ready) opens a modal with the fetched title/category/tags/description/chapters (cached client-side per slug, see [Configuration UI](#configuration-ui)).

**Manual editing**: `PATCH /episodes/{slug}/youtube` (`api.py:update_youtube_metadata()`) accepts any subset of `title`/`description`/`tags`/`category` and merges just those fields into the stored dict -- omitted fields are left untouched, so a title-only edit never touches the description Ollama generated. `title` is trimmed and clamped to YouTube's real 100-character title limit the same way generation does; `category` is validated against `youtube_publish.CATEGORY_IDS`; `tags` replaces the list wholesale and recomputes `tags_joined`. Chapters are deliberately **not** editable through this endpoint -- they're pure arithmetic derived from segment timings (see above), so hand-editing them would just get silently overwritten the next time the pipeline recomputes them, and there's nothing there a human could usefully correct anyway. 404s if metadata hasn't been generated yet for that episode (there's nothing to merge into). The Configuration UI's Episodes tab exposes this via an "Edit metadata" toggle inside the YouTube modal -- see [Episodes tab](#episodes-tab-search-edit-delete).

**Generating on demand**: `POST /episodes/{slug}/youtube/generate` (optional `{"force": true}`) calls the same `youtube_metadata.generate()` the pipeline itself calls, but directly -- independent of running a job -- for an episode that predates this feature, had it disabled at generation time, or whose one Ollama call failed. This is the same pattern as the voice-preview endpoint calling `voice_samples.get_or_create()` directly rather than through a job. In the Configuration UI, the "YouTube" button in the Actions column always opens the modal regardless of whether metadata exists yet -- if `GET /episodes/{slug}/youtube` comes back 404, the modal shows a "Generate YouTube metadata" prompt in place of the usual view/edit content (`youtubeNotGeneratedHtml()`/`wireYoutubeGenerateButton()`), rather than a separate button out on the row itself; a "Regenerate metadata" button inside the modal calls the same endpoint with `force: true` for an episode that already has it. A 502 means the underlying Ollama call failed or the episode has no completed segments yet -- check `history-api`'s logs for the actual error.
...
`POST /episodes/{slug}/youtube/publish` does one of two things depending on whether `video_id` is given:

- **`video_id` given** -- pushes this episode's metadata onto that already-uploaded video (`videos.update`). Fast, synchronous.
- **`video_id` omitted** -- uploads the episode's own finished MP4 as a brand-new video, with this metadata attached in the same call (`videos.insert`, via Google's resumable upload protocol). This is the normal path for an episode that's never been on YouTube at all -- no manual upload, no hunting for a video ID first.
...
**Updating an existing video**: `src/youtube_publish.py:update_video_metadata()` mints a short-lived access token from the refresh token, fetches the video's *current* snippet first and merges in the new title/description/tags/categoryId (rather than replacing the whole snippet from scratch, since `videos.update` overwrites the entire object -- this avoids silently wiping fields like `defaultLanguage` that this app never touches), and appends the chapters block to the description verbatim (YouTube parses chapter markers straight out of description text, there's no separate API field for them).

**Uploading a new video**: `src/youtube_publish.py:upload_video()` follows Google's resumable upload protocol -- `initiate_resumable_upload()` registers the snippet/status metadata and gets back a session URL (`POST .../upload/youtube/v3/videos?uploadType=resumable`, reading the `Location` response header), then `upload_video_file()` streams the MP4's bytes to that URL in one `PUT` (not chunked -- a dropped connection mid-upload means starting over, not resuming partway; acceptable for the hundreds-of-MB files this pipeline produces, worth revisiting if ever pointed at something much larger).

This runs as a **background thread**, not inline in the request handler: a several-hundred-MB upload over a modest home connection can take minutes, and `history-api` runs as a single `uvicorn` worker (see `compose.yaml`) -- blocking that worker's one thread for the whole upload would freeze every other request (job status polling, other episodes' pages, everything) for its entire duration. `POST /episodes/{slug}/youtube/publish` with no `video_id` returns `{"status": "uploading"}` immediately; `GET /episodes/{slug}/youtube/publish/status` is the poll target (`uploading` / `complete` with `video_id` / `error` with `error`), tracked in an in-memory `_youtube_publish_status` dict (lost on a server restart mid-upload, the same tradeoff `_jobs` already makes for episode generation). A second publish call while one is already in flight for that slug is a no-op (returns the current status without starting a duplicate upload). On completion, `video_id` and `published_at` are also persisted into the manifest's `"youtube"` block, same as the update-only path.

**Privacy status never defaults to public**: `privacy_status` (`private` / `unlisted` / `public`, only used for a fresh upload) defaults to **`private`** -- `youtube_publish.DEFAULT_PRIVACY_STATUS`. A video going live to the world with no confirmation step is exactly the kind of surprising, hard-to-reverse action this shouldn't do silently; the Configuration UI surfaces the choice explicitly rather than hiding it behind a default.

Category names map to YouTube's stable US numeric category IDs (`Education` -> 27, `Science & Technology` -> 28, `Entertainment` -> 24 -- the only three `propose_youtube_metadata`'s tool schema can produce), falling back to Education if something unexpected comes back. A 503 (not a stack trace) from either publish mode means the three OAuth env vars above aren't configured yet; a 502 (update) or an `"error"` status (upload) means the YouTube API call itself failed (bad video ID, revoked consent, quota exceeded, ...) -- the message is Google's own error text passed through.

**Description length**: YouTube hard-rejects `videos.update`/`videos.insert` with `400 invalidDescription` once `snippet.description` exceeds 5000 characters -- found live on a real 112-segment, 123-minute episode whose chapters list *alone* (one line per segment) was already 5403 characters, before even adding the summary text. `youtube_publish.py:_build_description()` combines the summary and chapters and, only if that combination is too long, truncates the chapters on whole lines from the end (never mid-line, never touching the summary) and appends a short note that some were omitted. A very long episode's early chapters -- the ones closest to a viewer's likely starting point -- are always kept intact; only the tail gets dropped. This is separate from `youtube_metadata.py`'s own `compute_chapters()`, which always returns the complete, untruncated list (so the Configuration UI's panel and the manifest's stored data show every chapter regardless of what actually fits in a given publish call) -- truncation is purely a publish-time adaptation to YouTube's API constraint, not a change to the generated data itself.

The Configuration UI's "YouTube metadata" panel (see [Access](#youtube-metadata) above) has a video-ID field, a privacy-status dropdown (used only when the ID field is left blank), and a single "Publish to YouTube" button that does whichever operation applies -- polling the status endpoint and showing a live "Uploading..." message for the upload path.

**Published/privacy badges**: once an episode has a `video_id`, its row in the Episodes tab shows a green "Published" badge (linking straight to the video) next to its title, and -- when known -- a Private/Unlisted/Public badge next to it (`youtubeBadgesHtml()` in `app.js`, styled via `.tag-published`/`.tag-privacy-*` in `style.css`; the same privacy badge also appears next to the "Last published" link inside the modal). These come straight from `_episode_progress()`'s `youtube_video_id`/`youtube_privacy_status` fields, which just read `manifest["youtube"]["video_id"]`/`["privacy_status"]` -- **not** a live YouTube lookup. `privacy_status` is only ever set by *this app's own* upload of a brand-new video (`_run_youtube_upload()` persists whatever YouTube's response confirms, in `result["status"]["privacyStatus"]`, falling back to the value requested if the response is missing it for some reason); pushing metadata onto an *existing* video (`video_id` given to `/publish`) never touches privacy at all, so that path shows the "Published" badge but no privacy badge unless a prior upload already set one. This means the badge can go stale if privacy is later changed by hand in YouTube Studio outside this app -- accepted deliberately, since a live per-episode YouTube API call on every 8s list poll would be wasteful and quota-risky for something this app only ever changes itself.

**Tests**: `tests/test_youtube_publish.py` (access-token refresh, snippet fetch/merge, chapter-appending, description-length truncation -- including a regression test built from the real 112-segment failure above -- category mapping and its fallback, the resumable-upload initiate/file-PUT steps and their HTTP-failure paths, and the full `upload_video()` orchestration including the private-by-default and explicit-privacy-status cases -- all against a fake `requests` module, no live Google API calls in the test suite) and the publish-endpoint cases in `tests/test_api_youtube_endpoint.py` (503 when unconfigured, 404s, the update path's success/502, and the upload path's success/error/already-in-flight/missing-file/invalid-privacy-status cases, using a fake `threading.Thread` that runs synchronously so the background upload's outcome can be asserted on directly).
...
**Title-card text shrinks to fit, rather than using one fixed size**: `EPISODE_TITLE_MAX_FONTSIZE` (60) and `SEGMENT_TITLE_MAX_FONTSIZE` (80) in `video.py` are ceilings, not fixed values -- `_fit_fontsize()` measures the actual title text with Pillow against the real font file (`FONT_PATH`) and shrinks the fontsize (down to `MIN_TITLE_FONTSIZE`, 32) until it fits within `TITLE_MAX_WIDTH_RATIO` (92%) of the 1920px frame width, leaving margin on both sides. This exists because a single fixed fontsize can't work for every title: a real 76-character episode title (`"Mastering Complex Numbers Algebra: A Journey Through the Hidden Dimensions"`) was found clipping off *both* edges of the frame at a fixed fontsize=60, rendering the start and end of the title completely unreadable. `ffmpeg`'s `drawtext` filter has no built-in shrink-to-fit of its own -- the fontsize used for each title is computed in Python before the `ffmpeg` command is even built, using Pillow's `ImageFont.getlength()` for real measurement against the actual font file rather than an approximate average-character-width guess (a rough per-character heuristic was tried first and found to give inconsistent results across even two real titles, since actual character widths vary a lot -- a capitalized, W/M-heavy title is measurably wider than an equally-long one full of narrow lowercase letters). Short titles are unaffected and still render at the full ceiling size; only long ones shrink, and only as much as needed to fit. **Tests**: `tests/test_video_title_fit.py` (short titles stay at the max fontsize, a long real title both shrinks below it and is verified to actually fit within the frame at its fitted size, and an absurdly long title bottoms out at `MIN_TITLE_FONTSIZE` rather than shrinking indefinitely).
...
## AI-Generated Episode Thumbnail

`visual_style="thumbnail"` (the fourth option alongside `static`/`math`/`images`) generates **one** AI image for the whole episode and reuses it as the constant background for every segment, instead of the plain color fill (`static`) or a per-segment sequence of cross-fading illustrations (`images`). Requested for episodes where per-segment images are more than wanted but a plain dark background feels too bare -- one representative "cover art" image throughout.

**Prompt derivation**: `src/prompts.py:build_thumbnail_prompt_system()`/`build_thumbnail_prompt_tool()` ask Ollama (`src/episode_thumbnail.py:_propose_prompt()`, a `chat_tool` call in the same shape as `ambient.py`'s mood design and `segment_images.py`'s per-segment scenes) for **one** vivid scene representing the episode's overall topic and title -- explicitly told to favor an iconic, representative scene over an event from any single part of the story, since this one image is on screen for the entire runtime, not just one segment. Same text-in-images and domain `image_style` handling as the per-segment case; if the Ollama call fails, the episode title (falling back further to the raw topic) is used as the scene description instead of failing the episode.

**Rendering**: `src/episode_thumbnail.py:generate()` renders the one scene via the same `image_client.generate_image()` FLUX.1-schnell path as per-segment images (same 1024x576 resolution, same negative prompt), and returns a `(rel_path, prompt)` pair for the caller to persist -- there's no per-image seeding here (unlike `segment_images.py`'s `_seed_for()`) since there's only ever one image per episode, generated once.

**One-time choice, not per-segment work**: `pipeline.py:apply_thumbnail_to_episode()` generates the image **once**, the first time an episode's `visual_style` becomes `"thumbnail"`, and caches the result in the manifest (`thumbnail_image`, `thumbnail_prompt`) -- reused on every subsequent run, including if `visual_style` is toggled away and back again, exactly the same one-time-choice philosophy as `math_visual`'s seeded random pick (see [Math Visualizations](#math-visualizations)). This runs alongside `apply_images_to_segments` (after narration/pitch, before YouTube metadata) rather than up front with the math-visual selection, since it needs the episode's real title, which isn't set until the outline call completes. ComfyUI's VRAM is freed (`image_client.free_memory()`) right after, same reasoning as the per-segment case.

**Compositing**: `video.py:build_episode_video()` passes the single cached image path as a length-1 `image_paths` list to every segment's `build_segment_video()` call -- this deliberately reuses `_images_input_and_filter()`'s existing single-image case (loop the one image for the segment's exact duration, no cross-fade) rather than adding a separate code path for a single still image. The same title/segment-title text overlay and legibility backdrop as every other non-static visual style is drawn on top. Gap clips between segments do **not** show the thumbnail image (same as `visual_style="images"`, whose gap clips also fall back to the plain background) -- only `math_bg_path` is wired into gap-clip rendering.

**Caching/invalidation**: switching `visual_style` away from `"thumbnail"` behaves like switching away from `"images"` or `"math"` -- the cached image is left on disk (harmless, reused if switched back), only the per-segment videos (which depend on the active style) are invalidated and rebuilt. See [Resumability](#resumability).

**Tests**: `tests/test_episode_thumbnail.py` (image generation and the Ollama-failure-falls-back-to-title behavior), `tests/test_pipeline_thumbnail.py` (the one-time-choice caching lifecycle: generates once, reused on resume, regenerated with `--force`, survives toggling `visual_style` away and back without a second Ollama/ComfyUI call), and `tests/test_video_thumbnail_visual.py` (a real small end-to-end render: both segments get a real background built from the one shared image, `visual_style != "thumbnail"` ignores a present `thumbnail_image`, and a missing `thumbnail_image` degrades to the plain static background rather than crashing). Run the same way as the pitch tests (see [Narrator Pitch](#narrator-pitch)).

## GPU / VRAM

Kokoro runs on **CPU** (see [OLLAMA.md](OLLAMA.md#text-to-speech-kokoro) for why — no official Blackwell/RTX 5090 support yet in the pre-built GPU image). FFmpeg's static-background encode is also CPU-only (no GPU filters/encoders used), as is the ambient bed's synthesis (`ambient.py:render()`, plain `lavfi` oscillator/noise filters) and the final ambient mix-in. This keeps `qwen3:32b` at 100% GPU throughout a generation run when `visual_style` isn't `"images"` or `"thumbnail"`, confirmed via `docker exec ollama ollama ps` during testing.

`comfyui` (used by `visual_style="images"` and `visual_style="thumbnail"`, see [AI-Generated Segment Images](#ai-generated-segment-images) and [AI-Generated Episode Thumbnail](#ai-generated-episode-thumbnail)) is the one GPU consumer that *does* share the RTX 5090 with `qwen3:32b` — its bundled PyTorch build (unlike the stable releases elsewhere in this stack) has working Blackwell (`sm_120`) kernels, so unlike Kokoro it didn't need to fall back to CPU. Because it shares the GPU, `gpu_lock.py`'s process-wide lock forces Ollama and ComfyUI calls to run strictly one at a time rather than concurrently, system-wide, across every job `history-api` has running — see the "Strictly sequential GPU scheduling" note above. Watch VRAM with `nvidia-smi` if running an `images`-style episode alongside heavy Ollama chat traffic.

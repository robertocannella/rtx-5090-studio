...
**The Actions column has a fixed width and scrolls horizontally instead of wrapping**: it can hold a lot (a YouTube button, its status text, "Edit", "Delete"), and letting it wrap onto a second line pushed that one row taller than every other row in the table, breaking the shared grid's otherwise-uniform row height. `.ep-col-actions` is `flex-wrap: nowrap` with `overflow-x: auto` and `min-width: 0` (needed because a grid item's default `min-width: auto` would otherwise refuse to shrink below its content's natural width, defeating the scroll) -- so a row with more buttons than fit just scrolls sideways within that one cell, and every row stays the same height regardless of how many actions it's showing.
...
**Deleting an episode**: "Delete" on a row (`startDeleteEpisode()`) replaces the action buttons with an inline "Delete this episode? ... Yes, delete / Cancel" confirmation -- no browser `confirm()` dialog, so it can't be dismissed by an accidental click-through the way a native dialog can. Confirming calls `DELETE /episodes/{slug}` (`api.py:delete_episode()`), which:

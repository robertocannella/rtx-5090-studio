...
**Editing YouTube metadata by hand**: inside the expanded YouTube panel, "Edit metadata" (`wireYoutubeEditMode()`) swaps the read-only title/category/tags/description into editable fields and a Save/Cancel row, saving via `PATCH /episodes/{slug}/youtube` (see [YouTube Metadata's Manual editing](#youtube-metadata)). Chapters stay read-only in both modes -- they're pure arithmetic, not something to hand-edit (see above). This is for the case where the Ollama-generated title/description needs a small human correction (a fact it got slightly wrong, a title you'd phrase differently) without burning another Ollama call via "Regenerate metadata."

...
def test_list_math_notes_posts_proxies_to_math_notes_api(client, monkeypatch):
    async def fake_request(self, method, url, **kwargs):
        assert method == "GET"
        assert url == f"{main.MATH_NOTES_API_URL}/posts"
        return _FakeResponse(200, [{"id": 1, "title": "A Post"}])

    monkeypatch.setattr(httpx.AsyncClient, "request", fake_request)
    resp = client.get("/api/math-notes/posts")
    assert resp.status_code == 200
    assert resp.json() == [{"id": 1, "title": "A Post"}]


def test_create_math_notes_post_proxies_with_body(client, monkeypatch):
    captured = {}
    ...
    monkeypatch.setattr(httpx.AsyncClient, "request", fake_request)
    body = {"title": "New Post", "category": "Physics", "body_markdown": "Body."}
    resp = client.post("/api/math-notes/posts", json=body)
    assert resp.status_code == 201
    assert captured["method"] == "POST"
    assert captured["url"] == f"{main.MATH_NOTES_API_URL}/posts"
    assert captured["json"] == body


def test_get_math_notes_post_proxies_to_the_right_path(client, monkeypatch):
    async def fake_request(self, method, url, **kwargs):
        assert method == "GET"
        assert url == f"{main.MATH_NOTES_API_URL}/posts/42"
        return _FakeResponse(200, {"id": 42})

    monkeypatch.setattr(httpx.AsyncClient, "request", fake_request)
    resp = client.get("/api/math-notes/posts/42")
    assert resp.status_code == 200


def test_update_math_notes_post_proxies_patch_with_body(client, monkeypatch):
    captured = {}
    ...
    async def fake_request(self, method, url, **kwargs):
        ...
        captured["url"] = url
        captured["json"] = kwargs.get("json")
        return _FakeResponse(200, {"id": 42, "title": "Updated"})

    monkeypatch.setattr(httpx.AsyncClient, "request", fake_request)
    resp = client.patch("/api/math-notes/posts/42", json={"title": "Updated"})
    assert resp.status_code == 200
    assert captured["method"] == "PATCH"
    assert captured["url"] == f"{main.MATH_NOTES_API_URL}/posts/42"
    assert captured["json"] == {"title": "Updated"}


def test_delete_math_notes_post_proxies_to_math_notes_api(client, monkeypatch):
    async def fake_request(self, method, url, **kwargs):
        assert method == "DELETE"
        assert url == f"{main.MATH_NOTES_API_URL}/posts/42"
        return _FakeResponse(200, {"status": "deleted", "id": 42})

    monkeypatch.setattr(httpx.AsyncClient, "request", fake_request)
    resp = client.delete("/api/math-notes/posts/42")
    assert resp.status_code == 200


def test_list_math_notes_categories_proxies_to_math_notes_api(client, monkeypatch):
    async def fake_request(self, method, url, **kwargs):
        assert method == "GET"
        assert url == f"{main.MATH_NOTES_API_URL}/categories"
        return _FakeResponse(200, ["Algebra", "Physics"])

    monkeypatch.setattr(httpx.AsyncClient, "request", fake_request)
    resp = client.get("/api/math-notes/categories")
    assert resp.status_code == 200

(() => {
  ...
  let mnPosts = [];
  let mnEditingPostId = null;

  async function loadMathNotesPosts() {
    const list = $("mn-posts-list");
    let posts;
    try {
      posts = await apiGet("/api/math-notes/posts");
    } catch (e) {
      list.innerHTML = `<p class="hint">Could not load posts: ${escapeHtml(e.message)}</p>`;
      return;
    }
    mnPosts = posts;
    if (!posts.length) {
    ...
    list.innerHTML = posts.map((p) => `
      <div class="job-card">
        <div class="job-title">${escapeHtml(p.title)} <span class="tag">${escapeHtml(p.category)}</span></div>
        <div class="job-meta">slug: ${escapeHtml(p.slug)} &middot; updated ${escapeHtml(new Date(p.updated_at).toLocaleString())}</div>
        <button type="button" class="small mn-edit-btn" data-id="${p.id}">Edit</button>
        <button type="button" class="small btn-danger mn-delete-btn" data-id="${p.id}">Delete</button>
        <a class="small" href="https://math.example.com/blog/${escapeHtml(p.slug)}/" target="_blank" rel="noopener" style="margin-left:8px;">View live &rarr;</a>
      </div>
    `).join("");
    list.querySelectorAll(".mn-edit-btn").forEach((btn) => {
      btn.addEventListener("click", () => startEditMathNotesPost(Number(btn.dataset.id)));
    });
    list.querySelectorAll(".mn-delete-btn").forEach((btn) => {
      btn.addEventListener("click", () => deleteMathNotesPost(Number(btn.dataset.id)));
    });
  }

  async function loadMathNotesCategories() {
    try {
      const categories = await apiGet("/api/math-notes/categories");
      $("mn-category-options").innerHTML = categories.map((c) => `<option value="${escapeHtml(c)}">`).join("");
    } catch (e) {
      // Best-effort -- the category field is still usable as free text without suggestions.
    }
  }

  function closeMathNotesEditor() {
    $("mn-editor-panel").style.display = "none";
    mnEditingPostId = null;
    $("mn-title").value = "";
    $("mn-category").value = "";
    $("mn-slug").value = "";
    $("mn-body").value = "";
    $("mn-save-status").textContent = "";
    $("mn-save-status").className = "";
  }

  function startNewMathNotesPost() {
    closeMathNotesEditor();
    $("mn-editor-heading").textContent = "New post";
    $("mn-slug-field").style.display = "";
    $("mn-editor-panel").style.display = "";
    $("mn-editor-panel").scrollIntoView({ behavior: "smooth", block: "start" });
  }

  function startEditMathNotesPost(id) {
    const post = mnPosts.find((p) => p.id === id);
    if (!post) return;
    mnEditingPostId = id;
    $("mn-editor-heading").textContent = `Editing: ${post.title}`;
    $("mn-title").value = post.title;
    $("mn-category").value = post.category;
    $("mn-slug").value = post.slug;
    // The slug is the post's permanent URL and can't be changed after creation (see
    // admin_app.py -- update_post() has no slug parameter at all), so hide the field
    // entirely while editing rather than show an input that would silently do nothing.
    $("mn-slug-field").style.display = "none";
    $("mn-body").value = post.body_markdown;
    $("mn-save-status").textContent = "";
    $("mn-save-status").className = "";
    $("mn-editor-panel").style.display = "";
    $("mn-editor-panel").scrollIntoView({ behavior: "smooth", block: "start" });
  ...
  async function saveMathNotesPost() {
    const title = $("mn-title").value.trim();
    const category = $("mn-category").value.trim();
    const slug = $("mn-slug").value.trim();
    const bodyMarkdown = $("mn-body").value;
    const status = $("mn-save-status");

    if (!title || !category || !bodyMarkdown.trim()) {
      status.textContent = "Title, category, and body are all required.";
      status.className = "error";
      return;
    }
    ...
    try {
      if (mnEditingPostId) {
        await apiPatch(`/api/math-notes/posts/${mnEditingPostId}`, { title, category, body_markdown: bodyMarkdown });
      } else {
        const payload = { title, category, body_markdown: bodyMarkdown };
        if (slug) payload.slug = slug;
        await apiPost("/api/math-notes/posts", payload);
      }
      closeMathNotesEditor();
      await loadMathNotesPosts();
      await loadMathNotesCategories();
    } catch (e) {
      status.textContent = "Failed: " + e.message;
      status.className = "error";
    }
  ...
  function deleteMathNotesPost(id) {
    const post = mnPosts.find((p) => p.id === id);
    if (!post) return;
    openConfirmModal({
      message: `Delete "${post.title}"? This permanently removes it from the live site.`,
      confirmLabel: "Yes, delete",
      onConfirm: async () => {
        await apiDelete(`/api/math-notes/posts/${id}`);
        await loadMathNotesPosts();
      },
    });
  }

  function wireMathNotesTab() {
    $("mn-new-post-btn").addEventListener("click", startNewMathNotesPost);
    $("mn-refresh-btn").addEventListener("click", () => {
      loadMathNotesPosts();
      loadMathNotesCategories();
    });
    $("mn-cancel-btn").addEventListener("click", closeMathNotesEditor);
    $("mn-save-btn").addEventListener("click", saveMathNotesPost);
  }
